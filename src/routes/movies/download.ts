    ...
    OnRequest: async function (req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (range) {
                ...
                // A request is considered COMPLETE only if it starts very close to the beginning 
                // (allowing for some HTTP headers) AND ends at the last byte
                const isFullDownload = start < 1024 && finalEnd >= fileSize - 1;
                
                // A request is considered a COMPLETION request if it's the very last chunk and small
                // This assumes browsers request files in sequential chunks
                const isLastChunk = start > (fileSize * 0.95) && finalEnd >= fileSize - 1 && 
                                    document.lastDownloadPosition && 
                                    start >= document.lastDownloadPosition;

                logger.info(`📥 Serving range: ${start}-${finalEnd}/${fileSize} (${Math.round((start / fileSize) * 100)}% already downloaded)`);

                // Save the current position for future comparison
                await collection.updateOne({ token }, { $set: { lastDownloadPosition: finalEnd } });
                ...
                // Mark as completed only if it's a full download or truly the final chunk
                if (isFullDownload || isLastChunk) {
                    res.on("finish", markAsCompleted);
                    logger.info(`⚠️ This appears to be the final range request (${isFullDownload ? 'full download' : 'sequential completion'})`);
                } else {
